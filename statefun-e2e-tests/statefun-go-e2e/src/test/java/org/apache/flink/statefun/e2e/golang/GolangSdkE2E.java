...
package org.apache.flink.statefun.e2e.golang;
 ...
 */
public class GolangSdkE2E {

  private static final Logger LOG = LoggerFactory.getLogger(GolangSdkE2E.class);
  ...
  private static ImageFromDockerfile remoteFunctionImage() {
    final Path goSdkSource = remoteGoSdk();
    LOG.info("Building remote function image with Golang source at: {}", goSdkSource);

    return new ImageFromDockerfile("remote-function")
        .withFileFromClasspath("Dockerfile", "Dockerfile.remote-function")
        .withFileFromPath("statefun-go/", goSdkSource);
  }

  private static Path remoteGoSdk() {
    return Paths.get(System.getProperty("user.dir") + "/../../statefun-go/");
  }
