...
final class Sinks {
  ...
  static Sinks create(StatefulFunctionsConfig config, StatefulFunctionsUniverse universe) {
    return new Sinks(config, sideOutputs(config, universe), sinkFunctions(universe));
  }
  ...
  private static Map<EgressIdentifier<?>, OutputTag<Object>> sideOutputs(
      StatefulFunctionsConfig configuration, StatefulFunctionsUniverse universe) {
    SideOutputTranslator sideOutputTranslator = new SideOutputTranslator(configuration, universe);
