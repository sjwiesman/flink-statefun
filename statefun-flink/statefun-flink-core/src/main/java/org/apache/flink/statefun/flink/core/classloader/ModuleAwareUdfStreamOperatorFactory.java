...
public class ModuleAwareUdfStreamOperatorFactory<OUT> extends SimpleOperatorFactory<OUT>
  ...
  public static <OUT> ModuleAwareUdfStreamOperatorFactory<OUT> of(
    ...
    try {
      ...
      throw new RuntimeException("Failed to serialize StreamOperator when building StreamGraph", e);
    }
  }
  ...
  ModuleAwareUdfStreamOperatorFactory(
  ...
  public <T extends StreamOperator<OUT>> T createStreamOperator(
    ...
    try {
      ...
      operator = serializedOperator.deserializeValue(moduleClassLoader);
    ...
    if (isInputTypeConfigurable && inputType != null) {
      ((InputTypeConfigurable) operator)
          .setInputType(inputType, parameters.getContainingTask().getExecutionConfig());
    }

    if (outputType != null) {
      ((OutputTypeConfigurable<OUT>) operator)
          .setOutputType(outputType, parameters.getContainingTask().getExecutionConfig());
    }
